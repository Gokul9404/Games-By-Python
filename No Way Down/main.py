...
# ============================================ Player ============================================
img_anim = {"forward": f"{path}/Sprites/Player/forward",
            "run": f"{path}/Sprites/Player/run", "idle": f"{path}/Sprites/Player/idle"}
img_anim_frame = {"forward": [12, 12, 12, 12],
                  "run": [12, 12, 12, 12], "idle": [30, 30]}
...
pygame.mixer.music.load(f"{path}/Music/background.wav")
...
def Home_screen():
    ...
    Start = PyButton(screen, "green", 150, 430, 100, 50, "Start",
                     'bookmanoldstlye', 35, Font_colour, Hovercolour, Command=Game_screen)
    Exitt = PyButton(screen, "red", 350, 430, 100, 50, "Exit",
                     'bookmanoldstlye', 35, Font_colour, Hovercolour, Command=exiit)
    Controls = Hover_button(screen, "white", 510, 340, 90, 20,
                            "Controls", 'bookmanoldstlye', 25, (220, 47, 2), bold=True)
    About = Hover_button(screen, "white", 510, 420, 90, 20,
                         "About", 'bookmanoldstlye', 25, (220, 47, 2), bold=True)
    i = 0
    Pressed = False
    while True:
        if i == 0:
            screen.blit(Home_screen_img[i], (0, 0))
...
def Game_screen():
    ...
    Restart = False
    Font_colour, Hovercolour = (35, 35, 35), (217, 217, 217)
    Home = Hover_button(screen, "white", 10, 5, 50, 25, "Home",
                        "californianfb", 20, Font_colour, Hovercolour, Command=Home_screen)
    Exit = Hover_button(screen, "red", 545, 5, 50, 25, "Exit",
                        'bookmanoldstlye', 35, (250, 25, 255), Hovercolour, Command=exiit)
    while True:
        Game_Surface.fill((145, 244, 255))
        # Load map of the level
        wall_tile, win_tile, win_tile_pos = load_map(
            Game_Surface, level, scroll)
        ...
        if Pos[1] <= 15:
            Scroll = True                                           # Start Scrolling
        # If scrolling check for stop or scroll
        if Scroll:
            if (Pos[1] >= 576) or (win_tile_pos[1] >= 45):
                # Stop Scrolling
                Scroll = False        
            else:
                # Scrolling speed  in px
                scroll[1] += 5
...
def level_upscreen():
    global level
    Font_colour, Hovercolour = (35, 35, 35), (217, 217, 217)
    Cont = PyButton(screen, "green", 150, 450, 100, 50, "Continue",
                    'bookmanoldstlye', 30, Font_colour, Hovercolour)
    Exitt = PyButton(screen, "red", 350, 450, 100, 50, "Exit",
                     'bookmanoldstlye', 35, Font_colour, Hovercolour, Command=exiit)
...
def end_screen():
    Font_colour, Hovercolour = (35, 35, 35), (217, 217, 217)
    Home = PyButton(screen, "green", 150, 430, 100, 50, "Restart",
                    'bookmanoldstlye', 30, Font_colour, Hovercolour, Command=Home_screen)
    Exitt = PyButton(screen, "red", 350, 430, 100, 50, "Exit",
                     'bookmanoldstlye', 35, Font_colour, Hovercolour, Command=exiit)
    while True:
        screen.blit(win, (0, 0))
