...
mixer.init()
# ============================================ Path for the assets =================================
# If path changed goto Load_map and set path their too
path = "E:/Python Programs/Games on PC/Tank_maniac/Assets"
# ============================================ Create the screen ============================================
...
def Enemy_controls(list_of_state: list, State_to_do: list, shoot: bool):
    ...
    if (thrust_per != set_thrust_per):
        if thrust_per > set_thrust_per:
            change_shoot_thrust = -1
        elif thrust_per < set_thrust_per:
            change_shoot_thrust = 1
    else:
        T = False
...
def game_requirements():
    ...
    """
    Home_but = Hover_button(Game_Surface, "white", 20, 10, 90, 20, "Home", 'Georgia',
                            20, (255, 55, 55), Hover_colour=(212, 21, 24), bold=True)  # ,(200,37,2)
    Exit_but = Hover_button(Game_Surface, "white", 800, 10, 90, 20, "Exit", 'Georgia', 20, (
        255, 55, 55), Hover_colour=(255, 5, 5), bold=True, Command=exiit)  # ,(200,37,2)

    health_bar_color = (90, 100, 80)
    plyr = Player(Game_Surface, 90, 350, 40, 400, type_char='player1')
    health_show_1 = Show_Text(
        Game_Surface, f"Health Player 1:{plyr.get_health()}", 18, health_bar_color, 20, 50, 'Georgia')
    enmy = Player(Game_Surface, 750, 350, 512, 872, type_char='player2')
    health_show_2 = Show_Text(
        Game_Surface, f"Health Player 2 :{enmy.get_health()}", 18, health_bar_color, 720, 50, 'Georgia')
    ...
    sht_per = '0'
    Shoot_thrust_percent = Show_Text(
        Game_Surface, sht_per, 34, (68, 69, 92), 360, 10, 'forte')
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYUP:
            ...
            if (event.key == K_UP) or (event.key == K_DOWN):
                Vessle_Rotating_direction = 0
            if (event.key == K_a) or (event.key == K_d):
                change_shoot_thrust = 0
...
def One_player_mode():
    ...
    while True:
        ...
        Game_Controlls = controls(Vessle_Rotating_direction, Movement_direction,
                                  change_shoot_thrust, quit_count, wait_over, exit_func_loop, shoot)
        if Player_Can_shoot and (Turn_no == 0):
            [Vessle_Rotating_direction, Movement_direction, change_shoot_thrust, shoot], [
                quit_count, wait_over, exit_func_loop, Go_home_screen] = Game_Controlls
        else:
            _,  [quit_count, wait_over, exit_func_loop,
                 Go_home_screen] = Game_Controlls
        # ==========================================================================================
        if Turn_no == 1:
            [Vessle_Rotating_direction, Movement_direction, change_shoot_thrust,
                shoot] = Enemy_controls(Players[Turn_no].get_states(), State_to_do, shoot)
            if shoot:
                Player_Can_shoot = True
        # ==========================================================================================
            ...
            if (wait_over and shooted):
                ...
                if Turn_no == 0:
                    ...
                    if Genrate_Enemy_controls:
                        Enemy_can_shoot = True
                        State_to_do = Generate_controls(
                            Players[Turn_no].get_states(True))
                        Genrate_Enemy_controls = False
                shooted = False
            # ======================================================================================
            if Vessle_Rotating_direction == 1:
                Players[Turn_no].rotate_gun_vessle('up')
            elif Vessle_Rotating_direction == -1:
                Players[Turn_no].rotate_gun_vessle('down')
        ...
        Draw_object(platform, Home_but, Exit_but, plyr, enmy, Cannon,
                    Shoot_thrust_percent, health_show_1, health_show_2)
        # ==========================================================================================
        # Keeping the Frames on Limit, And Updating the Display
        Clock.tick(FPS)
        # Updating the Game Surface/Screen
        if not exit_func_loop:
        ...
        if WINNER:
            ...
            if fin:
                Go_to_win_screen = True
        # ==========================================================================================
        Go_to_home = (Check_Mouse_button_down(Home_but) or Go_home_screen)
        # ==================================================================================================
        if exit_func_loop or Go_to_home or Go_to_win_screen:
            Del_objects(Home_but, Exit_but, plyr, enmy, health_show_1,
                        health_show_2, Cannon, Shoot_thrust_percent, platform)
        # ==========================================================================================
        if Go_to_home:
            Home_screen()
        if Go_to_win_screen:
...
def set_game_mode():
    ...
    gmtxt = '- Tank Battle -'
    Game_nm_text = Show_Text(Game_Surface, gmtxt, 70,
                             (20, 20, 20), 240, 130, 'forte')
    # ==============================================================================================
    Font_sizze = 24
    Font_type = 'Georgia'
    ...
    # ==============================================================================================
    option_text = Show_Text(Game_Surface, 'Select a mode', 30,
                            (120, 80, 60), 330, 300, Font_type, Bold=True, Italic=True)
    set_one_player = Hover_button(Game_Surface, "white", 350, 360, 180, 20, "Single Player", Font_type,
                                  Font_sizze, Font_colour, Hover_colour=Hover_color, bold=True, Command=One_Player_offline)
    set_two_offline = Hover_button(Game_Surface, "white", 350, 410, 180, 20, "Double Players", Font_type,
                                   Font_sizze, Font_colour, Hover_colour=Hover_color, bold=True, Command=Two_Player_offline)
    # ==============================================================================================
    quit_count = 0
    ...
    while True:
            ...
            if event.type == KEYDOWN:
                if event.key == K_q:
                    if quit_count == 2:
                        exit_func_loop = True
            if event.type == KEYUP:
                if event.key in [K_d, K_s]:
                    exit_func_loop, Pressed = True, True
                if event.key == K_s:
        ...
        if exit_func_loop or Pressed or Go_to_home or One_player_game or Two_player_game:
            Del_objects(set_one_player, set_two_offline,
                        Game_nm_text, option_text)
        # ==========================================================================================
        if Go_to_home:
            Home_screen()
        if One_player_game:
...
def Home_screen():
    ...
    Game_nm_text = Show_Text(Game_Surface, gmtxt, 70,
                             (20, 20, 20), 240, 130, 'forte')
    # ==============================================================================================
    Start = Button(Game_Surface, (200, 200, 200), 290, 370, 120,
                   45, "Start", 'bookmanoldstlye', 35, Font_colour, (15, 18, 25))
    Exitt = Button(Game_Surface, (250, 70, 70), 460, 370, 120, 45, "Exit",
                   'bookmanoldstlye', 35, Font_colour, (250, 0, 0), Command=exiit)
    # ==============================================================================================
    ...
    Show_Control = Hover_button(Game_Surface, "white", 400, 435, 90, 20,
                                "Controls", 'Georgia', 20, Font_colour, bold=True)  # ,(200,37,2)
    # Controls texts
    ...
    Plyr_movmnt_show = Show_Text(
        Game_Surface, Player_movement, 24, Control_color, control_x, 140, control_font_type)
    vssl_movmnt = "Vessle Movement :- UP/DOWN arrow key"
    vssl_movmnt_show = Show_Text(
        Game_Surface, vssl_movmnt, 24, Control_color, control_x, 185, control_font_type)
    power_chng = "Adjust Shoot Power :-  'A' / 'D' keys"
    Change_power_show = Show_Text(
        Game_Surface, power_chng, 24, Control_color, control_x, 230, control_font_type)
    go_home_shortcut = "Home shortcut :- 'H' key"
    go_home_shortcut_show = Show_Text(
        Game_Surface, go_home_shortcut, 24, Control_color, control_x, 275, control_font_type)
    sht = "Space Bar: To-Shoot/Start Game"
    shoot = Show_Text(Game_Surface, sht, 24, Control_color,
                      control_x, 320, control_font_type)
    qgt = "To exit :- Press 'Q'--> 3 Times"
    qgt_s = Show_Text(Game_Surface, qgt, 24, Control_color,
                      control_x, 365, control_font_type)
    # ==============================================================================================
    i, quit_count = 0, 0
    ...
    while True:
        ...
        if i == 0:
            ...
            Draw_object(ply, Plyr_movmnt_show, Change_power_show,
                        vssl_movmnt_show, go_home_shortcut_show, shoot, qgt_s)
        # ==========================================================================================
        # Taking the input from the user
        for event in pygame.event.get():
            if event.type == QUIT:
            ...
            if event.type == KEYUP:
                if event.key == K_SPACE:
                    exit_func_loop, Go_to_set_game_mode = True, True
                if event.key == K_q:
                    quit_count += 1
        # ==========================================================================================
        # Keeping the Frames on Limit, And Updating the Display
        ...
        if exit_func_loop or Go_to_set_game_mode:
            Del_objects(ply, Plyr_movmnt_show, Change_power_show, vssl_movmnt_show,
                        go_home_shortcut_show, shoot, qgt_s, Start, Show_Control, Exitt, Game_nm_text)
        # ==========================================================================================
        if Go_to_set_game_mode:
            set_game_mode()
        # ==========================================================================================
...
def win_screen():
    ...
    winner_text_show = Show_Text(
        Game_Surface, winnertext, 70, (60, 60, 80), 170, 140, 'forte')
    # ==============================================================================================
    Home_but = Button(Game_Surface, (200, 200, 200), 250, 370, 100,
                      45, "Home", 'bookmanoldstlye', 35, Font_colour, (15, 8, 35))
    ReStart_but = Button(Game_Surface, (200, 250, 200), 370, 370, 120,
                         45, "Restart", 'bookmanoldstlye', 35, Font_colour, (15, 18, 35))
    Exitt = Button(Game_Surface, (250, 70, 70), 530, 370, 100, 45, "Exit",
                   'bookmanoldstlye', 35, Font_colour, (250, 0, 0), Command=exiit)
    # ==============================================================================================
    quit_count = 0
